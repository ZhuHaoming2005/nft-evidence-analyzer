...
use analysis_core::{
    AnalysisError, ApiKeys, CandidateAnalysis, CandidateRegistry, ContractId,
...
fn with_rayon_pool<T>(
    ...
    run: impl FnOnce() -> Result<T, AnalysisError> + Send,
    ...
    T: Send,
{
    ...
    let pool = rayon::ThreadPoolBuilder::new()
        .num_threads(threads)
        .thread_name(|index| format!("analysis-cpu-{index}"))
        .build()
        .map_err(|error| AnalysisError::invalid(format!("rayon pool: {error}")))?;
    pool.install(run)
}
    ...
    let (loaded, downloaded) = thread::scope(|scope| {
        let handle = scope.spawn(|| {
            let runtime = tokio::runtime::Builder::new_multi_thread()
                .enable_all()
                .build()
                .map_err(|e| AnalysisError::http(format!("seed NFT runtime: {e}")))?;
        ...
        let downloaded = handle
            .join()
            .map_err(|_| AnalysisError::invalid("seed NFT download worker panicked"))?;
        Ok::<_, AnalysisError>((loaded, downloaded))
    })?;
...
where
    S: Send,
    Init: Fn() -> S + Send + Sync,
    Query: Fn(&mut S, ContractId, &mut HitGraph) -> Result<(), AnalysisError> + Send + Sync,
{
    let active = states
        .iter()
        .enumerate()
    ...
    analysis_core::set_inner_query_parallel(true);
    let outcomes = active
        .par_iter()
...
fn resolve_seed_states(
    ...
    seeds: &[SeedRecord],
    progress: &dyn ProgressObserver,
) -> Result<(Vec<SeedDedupState>, Vec<FailureRecord>), AnalysisError> {
    ...
    let resolved = seeds
        ...
        .map(|seed| {
            ...
            progress.add_completed(1);
            Ok::<_, AnalysisError>((seed.clone(), result))
        })
        .collect::<Vec<_>>()
        .into_iter()
...
fn query_seeds_staged(
    ...
    metadata_threshold: f64,
    progress: &dyn ProgressObserver,
) -> Result<SeedDedupBatch, AnalysisError> {
...
pub fn run(config: &RunConfig, progress: &dyn ProgressObserver) -> Result<(), AnalysisError> {
...
fn evidence_cache_path(config: &RunConfig) -> PathBuf {
    ...
    let legacy = config
        .output_dir
        .join(analysis_core::DEFAULT_EVIDENCE_CACHE_FILE);
...
) -> Result<DedupCacheParams, AnalysisError> {
    let input_fingerprints = config
        .inputs
        .iter()
        .map(|path| {
            let metadata = std::fs::metadata(path).map_err(|error| {
                AnalysisError::invalid(format!("fingerprint input {}: {error}", path.display()))
            })?;
...
fn load_seed_batch_from_cache(
    store: &ResidentStore,
    cache: &analysis_core::DedupCacheFile,
    cache_path: &Path,
    progress: &dyn ProgressObserver,
) -> Result<SeedDedupBatch, AnalysisError> {
    progress.set_stage("dedup");
...
fn try_load_dedup_cache_preflight(
    ...
    config: &RunConfig,
    seeds: &[SeedRecord],
) -> Result<Option<analysis_core::DedupCacheFile>, AnalysisError> {
...
fn run_inner(config: &RunConfig, progress: &dyn ProgressObserver) -> Result<(), AnalysisError> {
    let seeds = load_seeds_json(&config.seeds)?;
    ...
    let mut evidence = match &config.enrich_override {
        ...
        None => {
                ...
                && holder_only.is_empty()
            {
                ...
                let fetch_result = {
                    let mut on_bundle = |bundle: &EvidenceBundle| -> Result<(), AnalysisError> {
                        sink.push(bundle)
                    };
                    if missing.is_empty() {
                        Ok(AHashMap::new())
    ...
    analysis_core::ensure_output_layout(&config.output_dir).map_err(AnalysisError::from)?;
    ...
    let writer = thread::Builder::new()
        .name("analysis-cand-writer".into())
        .spawn(move || -> Result<(), AnalysisError> {
            while let Ok((rel, body)) = write_rx.recv() {
                write_candidate_json_bytes(&writer_out, &rel, &body)?;
            }
            Ok(())
        })
        .map_err(|e| AnalysisError::invalid(format!("spawn candidate writer: {e}")))?;

    // Err arm carries candidate identity so failures.jsonl is never unknown/unknown.
    let analyze_results: Vec<Result<CandidateAnalysisBatch, (String, String, AnalysisError)>> =
        owned_evidence
            .into_par_iter()
            .map(|(cid, bundle_owned)| {
                ...
                write_tx.send((rel, body)).map_err(|e| {
                    (
                        chain.clone(),
                        address.clone(),
                        AnalysisError::invalid(format!("candidate write queue closed: {e}")),
                    )
                })?;
    ...
    writer
        .join()
        .map_err(|_| AnalysisError::invalid("candidate writer thread panicked"))??;
    ...
    for result in analyze_results {
        match result {
            ...
            Err((_, _, AnalysisError::Cancelled)) => return Err(AnalysisError::Cancelled),
            Err((chain, address, e)) => {
    ...
    let reports = seed_dedups
        ...
        .map(|(seed, seed_id, dedup)| {
            ...
            let (rollup, analysis_ok) = build_seed_analysis_rollup(
                ...
                &seed.address,
                seed_analyses,
                analysis_core::DETAIL_CANDIDATES_REL,
            );
...
mod tests {
    use super::*;
    use analysis_core::parquet::write_report_golden_fixture;
    use analysis_core::{DEFAULT_METADATA_THRESHOLD, DEFAULT_NAME_THRESHOLD, SaleEvent};
    ...
    fn cached_truncation_retries_only_when_a_provider_failure_was_recorded() {
        ...
        old_value_flows
            .provenance
            .push(analysis_core::EvidenceObservation {
                source: "alchemy".into(),
    ...
    fn explicit_rayon_threads_use_a_run_local_pool() {
        ...
        let workers = with_rayon_pool(Some(3), || {
            Ok::<_, AnalysisError>(rayon::current_num_threads())
        })
        .unwrap();
        assert_eq!(workers, 3);
        assert!(with_rayon_pool(Some(0), || Ok::<_, AnalysisError>(())).is_err());
    }
    ...
    fn omitted_name_threshold_skips_name_dedup_stage() {
        let dir =
            std::env::temp_dir().join(format!("analysis_no_name_dedup_{}", std::process::id()));
    ...
    fn mid_query_cancel_propagates_without_complete_manifest() {
        let dir =
            std::env::temp_dir().join(format!("analysis_cancel_mid_query_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let parquet = dir.join("fixture.parquet");
        write_report_golden_fixture(&parquet).expect("fixture");
    ...
    fn fixture_run_with_mocked_enrich_writes_summary_keys() {
        let dir = std::env::temp_dir().join(format!("analysis_run_fixture_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let parquet = dir.join("fixture.parquet");
        write_report_golden_fixture(&parquet).expect("fixture");
        ...
        let enrich: EnrichOverride = Arc::new(|registry, store, progress| {
            ...
            for &cid in registry.candidate_contracts() {
                ...
                bundle.quality = analysis_core::EvidenceQuality {
                    ...
                    ..analysis_core::EvidenceQuality::default()
                };
    ...
    fn run_cancel_before_report_skips_complete_manifest() {
        let dir = std::env::temp_dir().join(format!("analysis_run_cancel_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let parquet = dir.join("fixture.parquet");
        write_report_golden_fixture(&parquet).expect("fixture");
        ...
        let enrich: EnrichOverride = Arc::new(|_, _, _| Err(AnalysisError::Cancelled));
        let err = run(
            &RunConfig {
                inputs: vec![parquet],
                seeds,
    ...
    fn run_writes_dedup_cache_and_reuse_skips_query() {
        let dir =
            std::env::temp_dir().join(format!("analysis_run_dedup_cache_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let parquet = dir.join("fixture.parquet");
        write_report_golden_fixture(&parquet).expect("fixture");
        ...
        run(&base_config(), &analysis_core::NoopProgress).expect("first run");
        ...
        run(&base_config(), &analysis_core::NoopProgress).expect("auto-reuse run");
        assert!(out.join("summary/all_chains.json").is_file());
        ...
        run(&base_config(), &analysis_core::NoopProgress)
    ...
    fn analyze_failure_records_candidate_identity() {
        let dir =
            std::env::temp_dir().join(format!("analysis_run_analyze_fail_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let parquet = dir.join("fixture.parquet");
        write_report_golden_fixture(&parquet).expect("fixture");

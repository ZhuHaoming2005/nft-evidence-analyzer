...
fn read_evidence_meta(path: &Path) -> Result<EvidenceCacheMeta, Analysis2Error> {
    let body = fs::read(path).map_err(|e| {
        Analysis2Error::invalid(format!("read evidence meta {}: {e}", path.display()))
    })?;
    serde_json::from_slice(&body).map_err(|e| {
        Analysis2Error::invalid(format!("parse evidence meta {}: {e}", path.display()))
    })
}

fn validate_cache_version(version: u32) -> Result<(), Analysis2Error> {
...
) -> Result<EvidenceCacheFile, Analysis2Error> {
    let mut by_key: AHashMap<(String, String), EvidenceBundle> = AHashMap::new();
...
fn load_sharded_evidence(
    ...
    for path in &paths {
        let compressed = fs::read(path).map_err(|e| {
            Analysis2Error::invalid(format!("read evidence shard {}: {e}", path.display()))
        })?;
        let body = zstd::stream::decode_all(std::io::Cursor::new(compressed)).map_err(|e| {
            Analysis2Error::invalid(format!("decompress evidence shard {}: {e}", path.display()))
        })?;
        let bundle: EvidenceBundle = serde_json::from_slice(&body).map_err(|e| {
            Analysis2Error::invalid(format!("parse evidence shard {}: {e}", path.display()))
        })?;
...
fn collect_evidence_shards(entries_dir: &Path) -> Result<Vec<PathBuf>, Analysis2Error> {
    ...
    while let Some(dir) = pending.pop() {
        ...
        for entry in entries {
            let entry = entry.map_err(|e| {
                Analysis2Error::invalid(format!("read evidence shard entry {}: {e}", dir.display()))
            })?;
            let path = entry.path();
            if path.is_dir() {
                pending.push(path);
...
pub fn migrate_evidence_cache_layout(
    path: &Path,
    cache: &EvidenceCacheFile,
) -> Result<EvidenceCacheMigrationStats, Analysis2Error> {
...
fn write_evidence_meta(path: &Path, params: &EvidenceCacheParams) -> Result<(), Analysis2Error> {
...
fn write_evidence_shard(
    entries_dir: &Path,
    bundle: &EvidenceBundle,
) -> Result<u64, Analysis2Error> {
    let portable = portable_bundle(bundle);
    let path = evidence_shard_path(entries_dir, &portable);
    let parent = path.parent().ok_or_else(|| {
        Analysis2Error::invalid(format!("evidence shard has no parent: {}", path.display()))
    })?;
    fs::create_dir_all(parent)?;
    let body = serde_json::to_vec(&portable)
        .map_err(|e| Analysis2Error::invalid(format!("serialize evidence shard: {e}")))?;
    let compressed = zstd::stream::encode_all(std::io::Cursor::new(body), EVIDENCE_ZSTD_LEVEL)
        .map_err(|e| Analysis2Error::invalid(format!("compress evidence shard: {e}")))?;
...
fn atomic_evidence_replace(path: &Path, body: &[u8]) -> Result<(), Analysis2Error> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)?;
    }
    let sequence = EVIDENCE_TEMP_SEQUENCE.fetch_add(1, Ordering::Relaxed);
...
fn clear_evidence_artifacts(path: &Path) -> Result<(), Analysis2Error> {
    for file in [
        path.to_path_buf(),
...
impl EvidenceCacheSink {
    ...
    pub fn finish(mut self) -> Result<usize, Analysis2Error> {
        self.flush()?;
...
mod tests {
    ...
    fn legacy_snapshot_is_reused_then_replaced_by_compressed_shards() {
        ...
        let a = store.contract_id("ethereum", "0xabc").unwrap();
        let dir = std::env::temp_dir().join(format!(
            "analysis2_evidence_legacy_migration_{}",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn legacy_jsonl_prefers_latest_row_and_migrates_without_snapshot() {
        ...
        let a = store.contract_id("ethereum", "0xabc").unwrap();
        let dir = std::env::temp_dir().join(format!(
            "analysis2_evidence_legacy_jsonl_{}",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn complete_sharded_write_replaces_obsolete_entries() {
        ...
        let d = store.contract_id("ethereum", "0xdef").unwrap();
        let dir = std::env::temp_dir().join(format!(
            "analysis2_evidence_complete_sharded_{}",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn interrupted_selective_refresh_keeps_previous_meta_day() {
        ...
        let a = store.contract_id("ethereum", "0xabc").unwrap();
        let dir = std::env::temp_dir().join(format!(
            "analysis2_evidence_refresh_meta_{}",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
